    ...
    textpos = []
		...
		@evolutions.length.times do |i|
			@evolutions[i].length.times do |v|
				...
				when :AttackGreaterSP then evo_txt = [_INTL("Lv. #{@evolutions[i][v][2]}"), _INTL("(Atk > SpAtk)")]
				when :SPAttackGreater then evo_txt = [_INTL("Lv. #{@evolutions[i][v][2]}"), _INTL("(SpAtk > Atk)")]
