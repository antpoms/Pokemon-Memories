...
#-----------------------------------------------------------------------------
  ...
  def pbStartScene(pokemon)
    ...
    def main
      ...
      @sprites["bgmap"].setBitmap("Graphics/Pictures/Pokemon Amie/"+pbBackdrop+".png")
      ...
      #holding food on the pokemon
        ...
        if @nibbleCooldown >= (Graphics.frame_rate*1)/2
          ...
          if @foodcounter==5
            ...
            @sprites["mouse"].oy=@sprites["mouse"].bitmap.height/2
            @mask=nil
            Mouse.instance_variable_set(:@hold, 0)
            Mouse.instance_variable_set(:@drag, nil)
            Mouse.instance_variable_set(:@static_x, Mouse.x)
            Mouse.instance_variable_set(:@static_y, Mouse.y)
            Input.update
            Graphics.update
          end
        else
